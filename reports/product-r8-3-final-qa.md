...
- Stability follow-up: Copilot R5 passed both isolated and adjacent-spec runs, classifying its prior full-matrix timeout as `RESOURCE_STARVATION`. The R3 portion collision was a `PARALLEL_INSERT_COLLISION`; its E2E fixture is now idempotent and has passed the Desktop/Mobile parallel matrix. No clinical-domain production code changed.
