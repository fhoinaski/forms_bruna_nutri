...
const sections = [
  ...
  ["Assistente de IA no portal do paciente", "O portal do paciente disponibiliza um assistente de inteligência artificial para ajudar a localizar e explicar informações do próprio acompanhamento — como plano alimentar, próxima consulta e tarefas — e para tirar dúvidas gerais sobre alimentação. Esse assistente responde apenas com os dados da própria pessoa autenticada, nunca de outros pacientes, e não acessa prontuário clínico completo, notas internas da nutricionista ou informações administrativas. Ele não altera o plano alimentar, não registra nada em prontuário, não diagnostica e não substitui avaliação profissional — qualquer sugestão de troca de alimento é apenas um ponto de partida para conversar com a nutricionista, e qualquer solicitação de agendamento só é confirmada após a própria pessoa revisar e aprovar na tela."],
  ...
  ["Analytics do site", "O site utiliza um sistema próprio de analytics (first-party), sem ferramentas de terceiros como Google Analytics ou Meta Pixel. São registrados dados técnicos anônimos/pseudônimos: página visitada, origem aproximada do acesso (ex.: busca, rede social, link direto), parâmetros de campanha (UTM) quando presentes, tipo de dispositivo e navegador de forma aproximada, e se o formulário de pré-consulta foi aberto, iniciado ou concluído. Não coletamos IP completo, não usamos fingerprinting invasivo (como identificação por fontes instaladas ou canvas), não gravamos tela nem digitação, e o identificador de sessão usado é um valor aleatório sem vínculo com nome, e-mail ou qualquer dado clínico — ele não é lido nem inspecionado no navegador do visitante. A finalidade é exclusivamente entender o desempenho do site e das campanhas (ex.: quais páginas são mais acessadas e quais canais trazem mais pessoas até a pré-consulta), nunca identificar uma pessoa específica enquanto ela navega anonimamente. Esses dados não são cruzados automaticamente com o cadastro de pacientes."],
    ...
    <main className="min-h-screen bg-[#FBF7F1] text-[#3A3028]">
      ...
      <section className="border-b border-[#EDE1D6] bg-[#FFFDFC] px-5 py-16 lg:px-8">
        <div className="mx-auto max-w-5xl">
          ...
          <p className="mt-6 max-w-3xl text-lg leading-8 text-[#75675E]">Esta política explica, em linguagem direta, como as informações são usadas e protegidas. Versão vigente: 18 de agosto de 2026.</p>
        </div>
      </section>
