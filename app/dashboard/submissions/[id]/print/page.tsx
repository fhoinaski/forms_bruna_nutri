...
export default async function PrintPage({
  ...
  const dateStr = formatDateSafe(submission.created_at);
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .info-value { font-size: 13px; color: #3A2B1F; }
      ...
      <div className="page">
        ...
        {ANSWER_SECTIONS.map((section) => {
          const visibleFields = section.fields
            .map((f) => ({ ...f, rendered: renderAnswer(submission.answers[f.key]) }))
            ...
            <div key={section.title} className="section">
              ...
              <div className="answer-grid">
                {visibleFields.map((f) => (
                  <div
                    key={f.key}
                    className={`answer-item${visibleFields.length === 1 ? " full" : ""}`}
                  >
                    <div className="answer-label">{f.label}</div>
                    <div className="answer-value">{f.rendered}</div>
                  </div>
                ))}
              </div>
            </div>
