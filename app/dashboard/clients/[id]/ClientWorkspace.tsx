...
import { PatientDeliverablesPanel } from "@/components/dashboard/PatientDeliverablesPanel";
...
export default function ClientWorkspace({
  ...
  const [temporaryPasswordMustChange, setTemporaryPasswordMustChange] = useState(true);
  ...
  const generateTemporaryPortalPassword = async (requirePasswordChange = true) => {
    setPortalLoading(true); setPortalError("");
    try {
      const response = await fetch(`/api/admin/clients/${id}/portal-access`, {
        method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "temporary_password", require_password_change: requirePasswordChange }),
      });
      const result = await response.json().catch(() => ({}));
      if (!response.ok || typeof result.temporary_password !== "string") throw new Error(result.message ?? "Não foi possível gerar a senha temporária.");
      setTemporaryPortalPassword(result.temporary_password);
      setTemporaryPasswordCopied(false);
      setTemporaryPasswordOpen(true);
      await reloadPortalAccess();
    } catch (err) {
      setPortalError(err instanceof Error ? err.message : "Não foi possível gerar a senha temporária.");
    } finally {
      setPortalLoading(false);
    }
  };
  ...
  const copyTemporaryPassword = async () => {
    if (!temporaryPortalPassword) return;
    try {
      await navigator.clipboard.writeText(temporaryPortalPassword);
      setTemporaryPasswordCopied(true);
    } catch {
      setPortalError("Não foi possível copiar a senha. Copie-a manualmente antes de fechar.");
    }
  };
  ...
  const requestTemporaryReset = () => { setTemporaryPasswordMustChange(true); setResetAccessOpen(false); setConfirmTemporaryResetOpen(true); };
  const confirmTemporaryReset = async () => { setConfirmTemporaryResetOpen(false); await generateTemporaryPortalPassword(temporaryPasswordMustChange); };
  useEffect(() => {
  ...
  return (
    <div className="mx-auto w-full min-w-0 max-w-7xl space-y-6 pb-16 animate-fade-up">
            ...
            <div className="space-y-6">
              ...
              {portalLoading && !portalAccess ? <p className="text-sm text-[#75675E]">Carregando acesso do portal...</p> : !portalAccess ? <p className="text-sm text-[#75675E]">O acesso ao portal será carregado ao abrir esta seção.</p> : (
                <div className="max-w-2xl space-y-4">
                  <PatientDeliverablesPanel patientId={id} />
                  ...
                  {(portalAccess.status === "NO_ACCESS" || portalAccess.status === "INVITE_PENDING") && <section className="rounded-2xl border border-[#EDE1D6] bg-[#FFFDFC] p-5"><p className="brand-kicker">Acesso ao portal</p><h3 className="mt-1 font-serif text-lg font-semibold text-[#3A3028]">Como deseja liberar o acesso?</h3>{portalAccess.status === "INVITE_PENDING" && <p className="mt-2 text-sm text-[#75675E]">Reenviar o convite invalida o link anterior.</p>}{!email && <p className="mt-2 text-sm text-[#9A5C4E]">E-mail necessário para enviar convite.</p>}<div className="mt-4 flex flex-col gap-3 sm:flex-row"><button type="button" disabled={portalLoading || !email} onClick={() => void generatePortalCode()} className="brand-btn-primary min-h-11">{portalAccess.status === "INVITE_PENDING" ? "Reenviar convite" : "Enviar convite por e-mail"}</button><button type="button" disabled={portalLoading} onClick={requestTemporaryReset} className="brand-btn-secondary min-h-11">Gerar senha de acesso</button></div></section>}
                  {portalAccess.status === "TEMP_PASSWORD" && <section className="rounded-2xl border border-[#EDE1D6] bg-[#FFFDFC] p-5"><p className="brand-kicker">Acesso</p><h3 className="mt-1 font-serif text-lg font-semibold text-[#3A3028]">Senha com troca obrigatória</h3><p className="mt-2 text-sm text-[#75675E]">A paciente precisará definir uma nova senha no primeiro acesso.</p><div className="mt-4 flex flex-col gap-3 sm:flex-row"><button type="button" disabled={portalLoading} onClick={requestTemporaryReset} className="brand-btn-secondary min-h-11">Gerar nova senha</button><button type="button" disabled={portalLoading} onClick={() => void togglePortalAccess(false)} className="brand-btn-secondary min-h-11">Revogar acesso</button></div></section>}
                  {portalAccess.status === "ACTIVE" && <section className="rounded-2xl border border-[#EDE1D6] bg-[#FFFDFC] p-5"><p className="brand-kicker">Opções do portal</p><h3 className="mt-1 font-serif text-lg font-semibold text-[#3A3028]">Acesso e segurança</h3><div className="mt-4 grid gap-3 sm:grid-cols-2"><div className="rounded-xl border border-[#EDE1D6] bg-[#FAF7F2]/60 p-4"><div className="flex items-start gap-3"><div className="rounded-lg bg-[#EAF0E4] p-2 text-[#607A56]"><ShieldCheck className="h-4 w-4" /></div><div><p className="font-semibold text-[#3A3028]">Sessões ativas <span className="text-[#607A56]">{portalAccess.active_sessions ?? 0}</span></p><p className="mt-1 text-xs leading-5 text-[#75675E]">Veja e encerre dispositivos conectados.</p></div></div><button type="button" onClick={managePortalSessions} className="brand-btn-secondary mt-4 w-full min-h-11"><ExternalLink className="h-4 w-4" /> Gerenciar sessões</button></div><div className="rounded-xl border border-[#EDE1D6] bg-[#FAF7F2]/60 p-4"><div className="flex items-start gap-3"><div className="rounded-lg bg-[#FFF3D8] p-2 text-[#9A6B20]"><KeyRound className="h-4 w-4" /></div><div><p className="font-semibold text-[#3A3028]">Credenciais de acesso</p><p className="mt-1 text-xs leading-5 text-[#75675E]">Envie um novo convite ou gere uma senha temporária.</p></div></div><button type="button" onClick={openResetAccess} className="brand-btn-secondary mt-4 w-full min-h-11"><RefreshCw className="h-4 w-4" /> Redefinir acesso</button></div></div><button type="button" onClick={() => void togglePortalAccess(false)} className="mt-3 inline-flex min-h-11 items-center justify-center rounded-full px-4 text-sm font-semibold text-[#9A5C4E] transition hover:bg-[#FFF7F5]">Revogar acesso ao portal</button></section>}
            ...
            <div className="space-y-6">
              ...
              <div className="grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">
                <div className="rounded-2xl border border-[#EAD8C2] bg-[#FAF7F2]/70 p-5">
                  ...
                  {(portalAccess?.status === "NO_ACCESS" || portalAccess?.status === "INVITE_PENDING") && <div className="mt-5 flex flex-col gap-3 sm:flex-row">
                    <button onClick={generatePortalCode} disabled={portalLoading} className="brand-btn-primary min-h-11">
                      <RefreshCw className="h-4 w-4" />
                      {portalAccess?.status === "INVITE_PENDING" ? "Reenviar convite por e-mail" : "Enviar convite por e-mail"}
                    </button>
                    <button type="button" onClick={requestTemporaryReset} disabled={portalLoading} className="brand-btn-secondary min-h-11">
                      <KeyRound className="h-4 w-4" />Gerar senha de acesso
                    </button>
                  </div>}
                  {portalAccess?.status === "TEMP_PASSWORD" && <div className="mt-5"><p className="mb-3 text-sm text-[#75675E]">A paciente precisará definir uma nova senha no primeiro acesso.</p><div className="flex flex-col gap-3 sm:flex-row"><button type="button" onClick={requestTemporaryReset} disabled={portalLoading} className="brand-btn-secondary min-h-11"><KeyRound className="h-4 w-4" />Gerar nova senha</button><button type="button" onClick={() => void togglePortalAccess(false)} disabled={portalLoading} className="brand-btn-secondary min-h-11">Revogar acesso</button></div></div>}
                  {(portalAccess?.status === "REVOKED" || portalAccess?.status === "LOCKED") && <div className="mt-5"><button onClick={generatePortalCode} disabled={portalLoading} className="brand-btn-primary min-h-11"><RefreshCw className="h-4 w-4" />Enviar novo convite por e-mail</button></div>}
                  {portalAccess?.is_active && portalAccess.status !== "TEMP_PASSWORD" && (
      ...
      {temporaryPasswordOpen && temporaryPortalPassword && (
        <div role="dialog" aria-modal="true" aria-labelledby="temporary-password-title" onKeyDown={trapDialogFocus} className="fixed inset-0 z-50 flex items-center justify-center bg-black/35 px-4 py-6 backdrop-blur-sm">
          <section className="w-full max-w-md rounded-3xl border border-[#EDE1D6] bg-[#FFFDFC] p-6 shadow-[0_28px_90px_rgba(58,48,40,0.24)]">
            ...
            <p className="mt-3 text-sm leading-6 text-[#75675E]">Esta senha será exibida somente agora. {temporaryPasswordMustChange ? "A paciente deverá criar uma nova senha no primeiro acesso." : "Ela poderá acessar o portal diretamente com esta senha."}</p>
      ...
      {confirmTemporaryResetOpen && (
        <div role="dialog" aria-modal="true" aria-labelledby="confirm-temp-reset-title" onKeyDown={trapDialogFocus} className="fixed inset-0 z-50 flex items-center justify-center bg-black/35 px-4 py-6 backdrop-blur-sm"><section className="w-full max-w-md rounded-3xl border border-[#EDE1D6] bg-[#FFFDFC] p-6 shadow-[0_28px_90px_rgba(58,48,40,0.24)]"><p className="brand-kicker">Credencial de acesso</p><h2 id="confirm-temp-reset-title" className="mt-1 font-serif text-2xl font-semibold text-[#3A3028]">Gerar senha para a paciente?</h2><p className="mt-3 text-sm leading-6 text-[#75675E]">As sessões ativas serão encerradas. Defina abaixo se esta senha deve ser trocada no primeiro acesso.</p><label className="mt-5 flex cursor-pointer items-start gap-3 rounded-xl border border-[#EAD8C2] bg-[#FAF7F2] p-4"><input type="checkbox" checked={temporaryPasswordMustChange} onChange={(event) => setTemporaryPasswordMustChange(event.target.checked)} className="mt-0.5 h-4 w-4 accent-[#607A56]" /><span><span className="block text-sm font-semibold text-[#3A3028]">Solicitar troca de senha no primeiro acesso</span><span className="mt-1 block text-xs leading-5 text-[#75675E]">Recomendado para uma senha temporária. Desmarque para permitir que a paciente entre diretamente com a senha gerada.</span></span></label><div className="mt-5 flex flex-col gap-3 sm:flex-row sm:justify-end"><button ref={resetAccessCloseRef} type="button" onClick={closeResetAccess} className="brand-btn-secondary min-h-11">Cancelar</button><button type="button" onClick={() => void confirmTemporaryReset()} className="brand-btn-primary min-h-11">Gerar senha</button></div></section></div>
      )}
      {deleteDialogOpen && (
