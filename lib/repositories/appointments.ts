...
export async function getMonthlyAppointmentCounts(months = 6): Promise<MonthlyCount[]> {
  ...
  const rows = await d1Query<{ month: string; c: number }>(
    `SELECT strftime('%Y-%m', starts_at) as month, COUNT(*) as c
     FROM appointments
     WHERE status != 'cancelado' AND starts_at >= ?1
     GROUP BY month`,
    [since.toISOString()]
  );
