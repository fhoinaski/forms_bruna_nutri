...
function createMiniV3(path: string) {
  ...
  `);
  const insertFood = db.prepare("INSERT INTO foods VALUES (?, ?, ?, 'USDA_SR_LEGACY', ?, 'https://fdc.nal.usda.gov/download-datasets', 'mini', 'test', 'COMPLETE', 0, 0)");
  ...
  const names = ["rice", "chicken", "salmon", "egg", "milk", "apple", "spinach", "bean", "almond", "olive oil"];
  for (let index = 0; index < 120; index += 1) {
    const base = names[index % names.length];
    const id = `food-${index}`;
    const name = `${base} pilot ${index}`;
    insertFood.run(id, name, name, String(100000 + index));
    insertNutrient.run(
      id,
      100 + index,
      400 + index,
...
describe("USDA pilot import script", () => {
  it("importa piloto local e comprova rerun idempotente", () => {
    const dir = mkdtempSync(join(tmpdir(), "usda-pilot-"));
    tempDirs.push(dir);
    const v3 = join(dir, "mini-v3.sqlite");
    const target = join(dir, "pilot.sqlite");
    const report = join(dir, "report.md");
    createMiniV3(v3);

    const output = execFileSync("node", [
      "scripts/usda-pilot-import.mjs",
      "--db", v3,
      "--import",
      "--pilot-size", "100",
      "--target", target,
      "--report", report,
      "--batch-id", "USDA_PILOT_TEST",
    ], { cwd: process.cwd(), encoding: "utf8" });
    const result = JSON.parse(output.match(/\{[\s\S]*\}/)![0]);
    expect(result.selected).toBe(100);
    expect(result.import.createdFoods).toBe(100);
    expect(result.rerun.createdFoods).toBe(0);
    expect(result.rerun.createdNutrients).toBe(0);
    expect(result.audit.integrity).toBe("ok");
    expect(result.audit.diffs).toBe(0);

    const db = new sqlite.DatabaseSync(target, { readOnly: true });
    expect(db.prepare("SELECT COUNT(*) AS c FROM food_catalog_usda_foods").get().c).toBe(100);
    expect(db.prepare("SELECT COUNT(*) AS c FROM food_catalog_usda_nutrients WHERE unit = 'mcg'").get().c).toBeGreaterThan(0);
    expect(db.prepare("SELECT COUNT(*) AS c FROM food_catalog_usda_nutrients WHERE value = 0").get().c).toBeGreaterThan(0);
    db.close();
  }, 20_000);
});
