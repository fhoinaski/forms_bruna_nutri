  ...
  return <section className="brand-card mt-6 overflow-hidden">
    ...
    <div className="p-7">
      ...
      {qrCode && <div className="grid gap-7 md:grid-cols-[260px_1fr] md:items-center">
        ...
        <div><h3 className="font-semibold text-[#3A2B1F]">1. Leia o QR Code</h3><p className="mt-2 text-sm leading-6 text-[#8A7B70]">Use Google Authenticator, Microsoft Authenticator, 1Password ou outro aplicativo TOTP.</p><p className="mt-4 text-xs text-[#8A7B70]">Chave manual</p><code className="mt-1 block break-all rounded-lg bg-[#F5ECE4] p-3 text-xs text-[#5F554D]">{manualKey}</code><label className="brand-label mt-5 block">2. Digite o código exibido</label><div className="mt-2 flex gap-2"><input value={code} onChange={(event) => setCode(event.target.value)} className="brand-input" inputMode="numeric" autoComplete="one-time-code" /><button onClick={verify} disabled={loading || code.length < 6} className="brand-btn-primary">Confirmar</button></div></div></div>}
